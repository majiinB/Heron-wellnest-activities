...
export class FlipFeelChoiceRepository {
  ...
  async create(question: FlipFeelQuestions, choice_text: string, mood_label: string) {
  ...
  async findById(choice_id: string) {
  ...
  async delete(choice_id: string) {
    return await this.repo.delete(choice_id);
  }
}
